 ...
 *         feature types returns json object with two elements: 
 *          keys: ["name", "start", "length", "trackIndex",
 *                  "features": {"num":<nfeatures>, "ids": <comma separated list of features assigned to the span"}]
 * 
 *          annot: [{"chr": <chrNo>, "annots":[[<name>, <start>, <length>, <features>], [], [], ..., []]} 
 */
...
public abstract class IdeogramJSONReporter extends AbstractReporter {
    ...
    final private static String FIELD_DISPLAY_ID = "display_label";
    ...
    final private static int TRACK_INDEX = 0;
    ...
    private JSONArray buildBinnedAnnotationJson(DataSource dataSource, ResultSet results) throws SQLException {
        ...
        while(results.next()) {
            ...
            List<Region> regions = createRegions(chr, size.longValue(), new JSONArray(results.getString(COLUMN_FEATURE_JSON)));
    ...
    private JSONArray buildFeatureAnnotationJson(JSONArray featureList) {
        JSONArray annotation = new JSONArray();
