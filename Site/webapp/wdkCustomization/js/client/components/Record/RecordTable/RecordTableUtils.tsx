...
import { StackedBar, NASpan } from "@components/Record/Attributes";
...
export const resolveNAs = (value: string, accessor?: any) => {
    if (value === "n/a") {
        return (
            <NASpan key={Math.random().toString(36).slice(2)}></NASpan>         
        );
    }
    return value;
};
...
export const resolveAccessor = (key: string, accessorType: ColumnAccessorType = "Default") => {
    switch (accessorType) {
        case "StackedBar":
            return (row: any) => <StackedBar value={row[key]} percentage={row[key] * 100} />;
        case "BooleanFlag":
