...
function QuestionForm(props: Props) {
    ...
    let fullContainerClassName =
        `${containerClassName || ""} ` +
        (question.parameters.every(({ type }) => type !== "filter") ? cx("", "default-width") : cx("", "wide-width"));
    ...
    let defaultOnClickDescriptionLink = React.useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        const descriptionSection =
            containerRef.current && containerRef.current.querySelector(`.${cx("DescriptionSection")}`);
    ...
    return (
        <div className={fullContainerClassName} ref={containerRef}>
            ...
            <form onSubmit={handleSubmit} noValidate={!validateForm}>
                ...
                <hr />
                <DatasetsUsed recordType={recordClass.urlSegment} targetName={question.urlSegment} />
            </form>
        </div>
    );
}

interface DatasetsUsed {
    recordType: string;
    targetName: string;
}
...
const DatasetsUsed: React.FC<DatasetsUsed> = ({ recordType, targetName }) => {
    const [datasetsUsedData, setDatasetsUsedData] = useState<ServiceResult[]>();

    useWdkEffect((service: WdkService) => {
        service
            ._fetchJson<ServiceResult[]>(
                "get",
                `/dataset/reference?record_type=${recordType}&target_type=question&target_name=${targetName}`
            )
            .then(setDatasetsUsedData)
            .catch(() => setDatasetsUsedData(null));
    }, []);

    return (
        <div>
            {!isEmpty(datasetsUsedData) && (
                <h2 className="ebrc-DescriptionSearchDatasetsHeader">Datasets used in this search</h2>
            )}
            <ul>
                {!isEmpty(datasetsUsedData) &&
                Array.isArray(datasetsUsedData) && //right now it's coming back as empty object if not configured
                    datasetsUsedData.map(({ dataset_id, description, name }) => (
                        <li key={dataset_id}>
                            <div>
                                <Link to={`/record/dataset/${dataset_id}`}>{name}</Link>
                            </div>
                            <div>{description}</div>
                        </li>
                    ))}
...
function ShowHideGroup(props: GroupProps) {
    ...
    return (
        <div className={cx("ShowHideGroup")}>
            ...
            <div className={cx("ShowHideGroupContent")}>{isVisible ? props.children : null}</div>
        </div>
    );
}
                    ...
                    <React.Fragment key={parameter.name}>
                        ...
                        <div className={cx("ParameterControl")}>
                            ...
                            {parameter.visibleHelp !== undefined && (
                                <div className={cx("VisibleHelp")}>{safeHtml(parameter.visibleHelp)}</div>
                            )}
                        </div>
                    </React.Fragment>
                ))}
    ...
    return (
        <div>
            <HelpIcon tooltipPosition={tooltipPosition}>
                Give this search strategy a custom name. The name will appear in the first step box (truncated to 15
                characters).
            </HelpIcon>
            <input
                type="text"
...
export function WeightInput(props: WeightInputProps) {
    ...
    return (
        <div>
            <HelpIcon tooltipPosition={tooltipPosition}>
                Give this search a weight (for example 10, 200, -50, integer only). It will show in a column in your
                result. In a search strategy, unions and intersects will sum the weights, giving higher scores to items
                found in multiple searches. Default weight is 10.
            </HelpIcon>
            <input
                type="text"
                pattern="[+-]?\d*"
    ...
    return !props.description ? null : (
        <div className={cx("DescriptionSection")}>
            <div className={cx("Description")}>
                <hr />
                <h2 className={props.navigatingToDescription ? "ebrc-DescriptionSearchDescriptionHeader" : undefined}>
                    Description
                </h2>
    ...
    return (
        <div className={className}>
            ...
            <SearchNameInput
                ...
                handleCustomNameChange={handleCustomNameChange}
            />
            <WeightInput tooltipPosition={tooltipPosition} weight={weight} handleWeightChange={handleWeightChange} />
        </div>
    );
}
