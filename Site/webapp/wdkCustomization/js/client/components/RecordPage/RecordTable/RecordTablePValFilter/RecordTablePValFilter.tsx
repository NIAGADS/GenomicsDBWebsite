...
import { chain, entries, groupBy, map, filter } from 'lodash';
...
const PvalFilter = class extends React.Component<PvalFilterProps, PvalFilterState>{
	...
	componentDidMount = () => {
		...
		_drawAxes(svg, canvasSpec.height, _buildXAxis(xScale, this.maxP), _buildYAxis(yScale));
		_drawLabels(svg, canvasSpec);
		_drawSlider(svg, xScale, unXScale, smallestP, this.defaultP, canvasSpec, this.props.onChange);
	}

	render = () => {
		const { values, onChange } = this.props;
		return <div className='p-val-filter-chart'><svg className={this.props.selectClass}></svg></div>
	}
}
...
interface ChartDatum { pValue: string, count: number }
...
const _drawFrame = (selectorClass: string, cs: CanvasSpec) => {
	return d3.select(`.${selectorClass}`)
...
const _drawRectangle = (svg: d3.Selection<any>, xScale: d3.scale.Ordinal<string, number>, minXVal: number, height: number, classname: string = '') => {
...
const _drawCircle = (svg: d3.Selection<any>, xScale: d3.scale.Ordinal<string, number>, xVal: number, hcp: number, height: number, drag: any) => {
	svg.append('g')
		.append('circle')
		.attr('fill', 'orange')
		.attr('r', 7)
...
const _drawSlider = (svg: d3.Selection<any>,
...
const _transformData = (data: { [key: string]: any }) => {
	const stodEntriesSort = ([k, v]: any, [l, w]: any) => scientificToDecimal(k) > scientificToDecimal(l) ? 1 : -1
	let count = 0;
	return chain(data)
		...
		.map(([k, v]) => ({ pValue: String(Math.abs(+k.split('e')[1])), count: count += v.length }))
		.value();
}

const _getSmallestP = (values: { [key: string]: any }[]) => {
	return Math.abs(d3.max(values, d => +d.pvalue.split('e')[1]));
}
