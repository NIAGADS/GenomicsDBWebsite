...
const PValueChartFilter: React.FC<PvalFilterProps> = ({
    defaultValue,
    filterValue,
    minValue,
    maxValue,
    ...
    useEffect(() => {
        const svg = _drawFrame(target, canvasSpec),
            xScale = _buildXScale(canvasSpec.width, minValue, maxValue),
        ...
        _drawSlider(svg, xScale, defaultValue, canvasSpec, maxValue);
...
const _buildYAxis = (yScale: AxisScale<any>) =>
    axisLeft(yScale)
        .ticks(5)
        .tickFormat((d) => formatNumber(+d)).tickPadding(15);
...
function _buildDrag(this: any, xScale: ScaleLinear<number, number>, sizerClass: string, cb: number) {
    return drag().on("drag", function () {
        ...
        if (x >= xScale.range()[0] && x <= xScale.range()[1]) {
            ...
            debounce(() => Math.pow(10, xScale.invert(x)))();
        }
    });
}
