...
const GeneRecordSummary: React.SFC<IRecordHeading & StoreProps> = props => {
	...
	return <React.Fragment>
		...
		<div className="gene-record-summary-container">
			...
			<ul>
				<li><h4 className="subtitle">{record.attributes.gene_name}</h4></li>
				<li><span className="label">Location</span>: {record.attributes.span} </li>
				<li><span className="label">Gene Type</span>: {record.attributes.gene_type}</li>
				<li className='badge-links'><span className="label">More Information</span>:
				<RecordLinkOuts linkOutJson={props.record.attributes.third_party_data_links} type='badge' direction='horizontal' />
				</li>
				{record.attributes.ad_evidence_flag && <li><span className="label">Genetic Evidence for AD?</span>&nbsp;YES</li>}
			</ul>
			<div className="sub-categories">
				<LabeledLinkOuts linkOutJson={props.record.attributes.record_link_outs} direction='vertical' />
			</div>
		</div>
		{/*<div className='col-8'>
...
const LabeledLinkOuts: React.SFC<LabeledLinkOuts> = props => {
	const { linkOutJson, direction } = props,
		classnames = 'link-list d-flex' + direction === 'vertical' ? 'flex-column' : '',
		linkoutObj = resolveJsonInput(linkOutJson) as unknown as LabeledLinkOutGroup;
...
const RecordLinkOuts: React.SFC<RecordLinkOuts> = props => {
	const { linkOutJson, type, direction } = props,
		...
		classnames = 'link-list d-flex' + direction === 'vertical' ? 'flex-column' : '';
	return <ul className={classnames}>{input.map(link => <li key={link.url}><RecordLinkOut link={link} type={type} /></li>)}</ul>
}
...
const RecordLinkOut: React.SFC<RecordLinkOut> = props => {
	const { link, type } = props,
		classnames = type === 'badge' ? 'button-link button-link-small' : '';
	return withTooltip(<a className={classnames} href={link.url}>{link.value}</a>, link.tooltip, 'd-flex');
} 